...
| 22 | `RetrieveUserQuotaSummary` over a loopback port with a CSRF token, endpoint found by scanning the process table | `usage-source.ts`, `quota-harvest-cache.ts` | **I** — a private RPC of the vendor's language server. No CLI surface exists for it, established by enumeration in finding 7; siblings exist but sit outside the credential boundary (finding 8). Probed turn-free against our own child in finding 12 |
