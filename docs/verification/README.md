...
The current tree is the subagent-model-route change (`b023d21`) and the documentation commit on top of it, neither pushed yet. It passes a local gate: `pnpm verify:local` exits `0` on three consecutive runs. Current focused test counts are Core `182`, Project Memory `77`, Claude `6`, Antigravity `62`, Codex `78`, Suite `16`. (Reaching a green gate at all first took one fix — the first re-validation returned FAIL, PASS, PASS on a load-sensitive Project Memory recovery read race, itemized in `HANDOFF.md`; the `209`/`61`/`7`/`7` counts recorded at that point are themselves superseded by the Model Accounts removal, the Codex thread-resume redesign and the Antigravity catalog/diagnostic/usage rework since.)
