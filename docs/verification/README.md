...
Two things this run did NOT establish. Usage & Limits with all providers mounted is only half-served — Codex reports live figures, Antigravity reports unavailable, which is its documented pre-turn state — so that row stays open. And this is one session, driven by the same party that wrote the code; it is acceptance evidence, not independent validation.
