...
Added 2026-09-04, after the bounded turn retry that finding 18 of `agy-cli-contract.md` argues for: **all seven Antigravity live suites PASS on real `agy 1.1.26`**, each read from its own exit code -- `test:live:primary` (`8/8`), `test:live:agent-allowlist` (`1/1`), `test:live:session-continuation` (`1/1`), `test:live:quota` (`2/2`), `test:live:agent-loop` (`4/4`), `test:live:web-search` (`1/1`) and `test:live:web-search-routed` (`1/1`) -- 18 tests, no failures. `pnpm verify:local` exits `0`; the Antigravity focused count is `133`. **The vendor self-updated `1.1.25` -> `1.1.26`, so this is the first live evidence on that build**, and re-running was owed on that alone under the freeze's own terms.

Two things about the change this run followed. It came from a **production failure, diagnosed out of the vendor's own on-disk state rather than from a probe**: two concurrent DSH subagent sessions, one of which lost a step to a `status: ERROR` the vendor had already recovered from internally -- it injected "The stream was interrupted. Please continue the task you were working on.", re-called the backend, and recorded a valid decision for that same turn in `conversations/<id>.db`, which DSH never saw because it had killed the child. The timeline, the 503 that shed the request (4 of 1802 vendor logs on this machine have ever carried one, and exactly one of those hit a model call), and the store paths are finding 18. The sibling session's failure was the already-known stale-`structured_output` mechanism, triggered by a ~60 KB tool-result dump, with the vendor killing its own `agent_state` subscriber for falling 8 s behind at that exact millisecond.

**And these suites cannot provoke it either.** A shed model call is weather; nothing in the tree can ask for one. So the same distinction as the repair entry above applies: the seven suites show the retry broke nothing live, while the retry's own boundaries are pinned by unit tests -- ten of them, five of which fail when the retry is reverted -- and its justification is the vendor's recorded state, not a live assertion. The one live fact that IS new is negative and worth having: 18 scenarios of ordinary turns on `1.1.26` produced no `ERROR` settlement at all, so the retry never fired during the run.

Added 2026-09-04, after a rebuilt `full` envelope started naming the current request as `task` (bridge protocol `v4` -> `v5`): **all seven Antigravity live suites PASS on real `agy 1.1.26`**, each read from its own exit code -- `test:live:session-continuation` (`1/1`), `test:live:primary` (`8/8`), `test:live:agent-allowlist` (`1/1`), `test:live:quota` (`2/2`), `test:live:agent-loop` (`4/4`), `test:live:web-search` (`1/1`) and `test:live:web-search-routed` (`1/1`) -- 18 tests. `test:live:web-search` failed the first pass with `raw.sources.length > 0` (the vendor returned an empty source list) and passed on the immediate retry; the other six were green on the first pass. The Antigravity focused count is `138`.

**What this run does NOT establish**: none of the seven rebuilds a conversation that already contains this route's own replies, so none of them send a `task` field. `session-continuation` is still the load-bearing one for the protocol bump -- two turns in one child, the first a cold `full` (no `task`) and the second a `delta` -- and it passed, which is the assertion that a `v5` agent definition still answers a delta. The rebuild path is pinned by unit tests (`session-reuse.test.ts`, two cases that fail when `task` is omitted or names the opening question). The production failure this followed is session `session-8801c50c`: after a host restart a 62-character follow-up arrived as the last entry of a 62 k-token `messages` array and the model restarted the opening task; the same question sent again worked.

Added 2026-09-04, Grok routed native `web_search`: `packages/grok` focused tests `93/93`, `pnpm check` and `pnpm build` exit `0`. Live against real `grok 1.0.13` / `grok-4.5` low, each read from its own exit code: `test:live:web-search` `1/1`, `test:live:web-search-routed` `1/1`. The first routed pass failed `result.sources.length > 0` in 4 s (structured output arrived with an empty `sources` array after a native `web_search`); the backend now fills empty structured sources from the native `WebSearch` citations, pinned by a unit test, and the suite passed on the next run in 34 s. Finding 20 of `grok-cli-contract.md` is the contract read this backend rests on: client-side `web_search` leaves `web_search_requests` at 0, so the Messages stream is what proves the native tool ran. Primary isolation is unchanged (`--disable-web-search` stays on the chat turn).
...
**The retry bound counted the wrong thing.** `MAX_IGNORED_RETRYING_ERRORS = 2` was a LIFETIME cap: a turn that hiccupped once, recovered, streamed for a minute, hiccupped again and finished was killed on its third hiccup however far apart they fell. That is not what the change set out to bound — its own rationale is a turn that "sits until `turnTimeoutMs` and still bills", which is thrashing, not a tally. The count is now **consecutive** and any notification belonging to the turn resets it. Two tests pin both halves: six retrying errors each separated by progress must complete, and three in a row must still fail even after earlier progress. The threshold of two is unchanged and still unmeasured; making it consecutive is what removes the false kill.

**Isolation depends on two fields the vendor never declared.** `config/read` is where isolation learns what to disable, and it fails closed on a shapeless `mcp_servers`/`apps` — correctly, because silently disabling nothing would leave vendor tools on. But neither field is in the published contract: `codex app-server generate-json-schema` types `ConfigReadResponse.config` as `Config`, and that definition lists 25 keys — `model`, `model_context_window`, `web_search`, `tools` among them — with **no `apps` and no `mcp_servers`**. The vendor sends them and never promised them, so a change there cannot be caught by reading the schema, and the `apps: null` regression was not. Acceptance **scenario 16** is now the monitor that replaces the contract we do not have: it opens a real App Server, reads `config/read`, and asserts both fields are present and are a map or unset. It prints what it saw (`mcp_servers: object`, `apps: null`) and says in its own failure text that tripping means the vendor moved, not that the product broke.

**Withdrawn: the objection to failing on a non-string delta.** `AgentMessageDeltaNotification` types `delta` as a required non-nullable string, so rejecting anything else is the contract being enforced rather than an assumption — the opposite of the `apps` case, where the field is undeclared. Recorded because the two looked alike and are not.

Codex 162 -> 164 unit tests, acceptance 9 -> 10 scenarios. Re-verified end to end against real `codex-cli 0.150.0`: `acceptance` 10/10, `primary`, `inject-items`, `tool-result-continuation`, `web-search`, `web-search-routed` 1/1 each, workspace build/check/test all 0.
