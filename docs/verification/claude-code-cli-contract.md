...
**Probed on 2026-09-03**, after the first read of this file and before any route code: all five open probes ran against the installed 2.1.246 on `claude-haiku-4-5`, and the results are in *Probe results* below. The inventory now carries measurements where it carried guesses; what is still unmeasured says so.
...
1. **`-p` executes repository content without a trust prompt.** The vendor's own page: without `--bare`, a `-p` session "runs the hooks in a project's `.claude/settings.json` and connects the servers in its `.mcp.json`, even in a folder you've never trusted", with no workspace-trust dialog and no per-server approval, and settings files that fail validation are silently ignored. A primary route invokes this in the user's own workspace, so the exposure is real and it is ours to close. `--bare` cannot be the answer (condition 2). The candidate answer WAS `--safe-mode` plus `--strict-mcp-config` and an explicit `--settings`/`--setting-sources`; finding 3 probed it and safe mode disconnects MCP, so the answer holds only for a route that wants no MCP server -- which the thin route does not. The trap itself remains unprobed in a real repository, and the `-p` help states in the binary's own words that invalid settings files are ignored silently.
...
All five open probes ran, plus five findings nobody asked for. Shape: `claude 2.1.246`, `claude-haiku-4-5-20251001`, `--effort low`, a sterile workspace outside this repository, and a **scrubbed environment** -- only `PATH`, `HOME`, `USER`, `LANG`, `TERM=dumb`, because this session's own `CLAUDE_CODE_*` variables (including a messaging socket and token) would otherwise be inherited by every child and the probe would measure the harness. Total spend across every arm was well under a dollar. Recipes are not retained as a suite; they are described below.
...
- Whether a `-p` session in a real repository picks up that repository's hooks and `.mcp.json` as the vendor's own `-p` help warns (trap 1). Every arm above ran in an empty workspace, deliberately, so the trap is confirmed only as published text -- and finding 3 has already removed the remedy this file proposed for it.
...
4. **The isolation posture needs a second design pass, and it is now the only real open question.** `--safe-mode` was the answer to trap 1 and finding 3 removed it for any route wanting MCP. For the thin route the substitute is the flag set that does not need safe mode -- `--strict-mcp-config` with no `--mcp-config`, an explicit `--settings` and `--setting-sources`, `--tools ""` -- but that leaves the repository's own hooks question (trap 1) unanswered and unprobed, and it is unanswered in the direction that matters: the vendor's `-p` help says settings that fail validation are ignored **silently**.
