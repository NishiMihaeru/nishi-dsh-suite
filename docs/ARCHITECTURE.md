...
What that run exposed is not in delegation but in Codex: a turn whose first step runs on another provider and emits tool calls, and whose next step resolves on `codex-app-server`, fails with `codex-plugin-dsh: the current Codex turn has no user input`. The rule above — current-turn input is decided by position, and a tool result is never that input — leaves the pending tail as tool results alone when no Codex checkpoint exists in the session. The turn is lost; the session is not, and the following turn runs normally. Open, tracked in `ROADMAP.md` §2.
