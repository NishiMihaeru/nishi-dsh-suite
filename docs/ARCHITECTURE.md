...
Two transports exist for this route, selected by `transport`. `schema` is described above: `agy` is stripped to a model endpoint and its reply forced through `--json-schema`. `mcp-bridge`, **the default since its live acceptance passed**, hands the DSH catalog to the vendor's own harness as MCP tools so the model calls them natively. Making it the default makes the one-time registration a deployment requirement: the first turn fails with the exact command in the message, and never falls back on its own, because a route that silently hands the model no tools looks healthy.

The precondition checks the **grant** and not only the registration, and that is not belt-and-braces. A server registered without `mcp(<server>/*)` is the worst available state: the vendor launches it, the adapter claims it, and the MCP tools are absent from the model's toolset with no denial event anywhere. Measured live in that state, the model listed its tools as `manage_task, schedule, send_message, finish` and the turn returned an empty string with `status: SUCCESS`. A `disabled` server is refused for the same reason. The vendor's configuration is read and never written, and an unreadable or unexpected file counts as unknown rather than as a missing grant -- the layout is the vendor's to change, and a layout change must not be able to kill a working route. Beyond what any precondition can see, a finished turn whose bridge server never attached fails loudly: "the model made no tool call" is ambiguous, "no server ever attached" is not. All of this came out of an adversarial review that found the silent degradation; see `docs/verification/gemini/LATEST.md`.
...
Both providers therefore hold a vendor turn open across DSH's own tool execution, and both now assert that the turn is still open at the moment they answer it. Antigravity reads `openVendorTurn().settled()` before `bridge.resolve`; Codex scans its unread event queue with `ActiveTurnQueue.abandonedCall()` for a `dynamicToolCall` completion, a `turn/completed` for this turn, or a fatal non-retrying `error`. Either signal means the vendor stopped waiting, and answering into it would record a whole turn of reasoning built on a result the model never received — silent by construction, and observed in a real session before the guard existed. The step fails instead, and the next request realigns through the ordinary start-of-turn path. See `ROADMAP.md` section 5 for the evidence and for why Codex abandons a parked call at all.
...
Two asymmetries in the adapter are deliberate. A step that surfaces a tool call reports no usage, because the vendor has not finished counting the turn and a per-step figure would be counted again when the turn ends. And the turn timeout scopes the whole vendor turn rather than one DSH step, because a per-step timeout would kill a child while a tool was still running.

**The agent allowlist is `finish` alone, and must not name `call_mcp_tool`.** Probed on real `agy 1.1.22` after the obvious spelling killed every live turn: an allowlist naming the vendor's MCP wrapper terminates the agent outright -- `step_type: "error_message"`, zero tokens -- while the same definition without it runs. It is also unnecessary, because MCP tools are not gated by that list: with `tools: [finish]` the model reached a registered server anyway. So this transport's agent definition carries exactly the schema transport's allowlist and grants no native-tool exposure the shipped transport did not already have.

Isolation has two halves and both are now evidenced. The allowlist prevents: asked to read a file inside its own granted workspace, a `finish`-only agent invoked no native tool, leaked none of its contents, and reported having no filesystem or shell tools, while the turn still succeeded (`test:live:agent-allowlist`). `init.tools` reporting all 57 native tools is the CLI's registry, not the agent's effective set -- worth stating because it reads exactly like an ignored allowlist and prompted this check.

`BLOCKED_NATIVE_TOOLS` remains the post-hoc backstop on the vendor's event log, with one exemption on this transport: `call_mcp_tool`, which here is how a DSH tool is reached at all. That exemption was a defect the live run found -- the loop worked and the backstop then failed the turn -- and it is covered by a test asserting that `run_command` in the same position still trips the check.

Coverage: `test/mcp-bridge.test.ts`, including one test that drives the real server process over real MCP stdio and asserts the call stays blocked until DSH answers; `test/mcp-transport.test.ts`, which drives a full DSH step pair through the adapter against a fake vendor child and a fake bridge server; and `test:live:mcp-bridge`, which **passes** against real `agy 1.1.22`, asserting that the model read a value obtainable only from the tool result and that one vendor child and one vendor turn served the whole loop. `mcp-bridge` is the default; `transport: "schema"` selects the older path, which still carries the larger body of live evidence. Unit tests that assert forced-schema behaviour pin `transport: 'schema'` explicitly, and one pins the default itself.
