...
## Antigravity quota without a prior turn

Narrowing the quota harvest to this package's own child (the machine-wide process scan and its `--csrf_token` lifting were removed on 2026-09-03) left a real hole, recorded rather than glossed: no number existed until a turn had run, so a fresh session's usage panel showed an honest but empty row. It is now closed by a child spawned for the reading alone.

The shape is deliberately minimal: `--input-format`/`--output-format stream-json` and nothing else -- no model, no schema, no agent, none of which affect the listener and each of which would be another vendor surface a reading nobody asked a question for depends on. It runs in a scratch directory of its own, and **nothing is written to its stdin**, which is the entire guarantee that it costs no tokens: `agy` runs a turn per line of stdin, and there are no lines. Verified through this package's own discovery and cache rather than hand-rolled HTTP: two loopback listeners on the child's own pid at +261 ms, the real payload at +1.8 s, zero bytes on stdout while the reading is taken, no `init`, no `result`, no file in the vendor's `conversations/`, and an empty `crashes/` afterwards because disposal goes through the grace path rather than `SIGKILL`.

A cold start is cheap rather than free -- it is a real process, and it writes the ordinary vendor state any run writes -- so it is single-flighted, floored at one child per minute, deadlined at 15 s against that measured 1.8 s, and reached for only when no recent harvested reading exists. A cached reading still wins outright.

The question that decided whether any of this could ship was not whether the listener exists but whether a turn-free answer is **true**, and it took its own probes, because the first two cold reads disagreed wildly -- 43.89% used, then 0%. The disagreement was the Gemini weekly window rolling, visible in its reset moving from 2026-09-06 to 2026-09-10, not a placeholder: the live suite now reads one account both ways minutes apart and gets `usedPercent = 1.03` each time, the cold arm in 2.2 s. What is genuinely unreliable, and is not a cold/warm distinction at all, is the RPC's content: after an upstream `failed to get load code assist response`, every bucket can come back with no `remaining` field whatsoever -- persistently, and after a completed `SUCCESS` turn on the same child. That surfaces as an honest unsupported row only because `parseRetrieveUserQuotaSummary` skips a bucket whose fraction is not a finite number in `[0,1]` and throws when none survive. **That skip is load-bearing and must not be tidied away**: without it, a vendor hiccup reads as full quota. And none of it makes the number authoritative -- an Antigravity quota reading can report headroom while generation returns 429.
