...
The tool transport is the forced output schema. Names are pinned to the catalog; argument objects stay untyped, because a per-tool `anyOf` of full parameter schemas is what a real DSH session ended `end_turn` with no `structuredOutput` on, and `--json-schema` does not accept a file path. The reply still carries a per-step stamp. The envelope travels as `--prompt-file`, not `--prompt-json`: Linux kills a 128 KiB argv slot with `E2BIG`, which is how a rebuild after a failed decision died before `grok` started. DSH's system instruction rides in the envelope for the same reason — there is no `--system-prompt-file`. An embedded ACP `resource` was measured being read back verbatim in isolation and then treated as something to fetch in front of a 29-tool agent, so the envelope is inline text.
...
Usage & Limits is ACP `_x.ai/billing` over `grok agent stdio`: the same turn-free handshake the catalog already uses, plus one vendor-extension method, with no session. `/usage` is still a TUI billing action and is not this channel — passing it to the headless entry sends it to the model as prose. A reading with no finite percentage inside an open period is `UNAVAILABLE`, not a meter of zero. Prepaid/on-demand fields are not projected.

Routed `web_search` is a hidden headless turn, not the primary process. The primary argv still passes `--disable-web-search`. The search turn allowlists `web_search` only (`--tools web_search --disallowed-tools Agent,web_fetch --deny MCPTool`), never `--always-approve`, and reads `--output-format streaming-messages-json` so native search can be proven: a successful client-side `web_search` leaves `usage.server_tool_use.web_search_requests` at 0, because that counter is backend-hosted search only. `--json-schema` still binds on that stream. The search agent is pinned to `grok-4.5` at `low` effort regardless of the session primary: retrieval is the native tool, and a 4.6/xhigh session must not spend that on the wrapper. ACP `x.ai/search/*` is session/file fuzzy search and is not this backend.
