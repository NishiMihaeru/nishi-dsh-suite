...
Coverage: `test/mcp-bridge.test.ts`, including one test that drives the real server process over real MCP stdio and asserts the call stays blocked until DSH answers, and `test/mcp-transport.test.ts`, which drives a full DSH step pair through the adapter against a fake vendor child and a fake bridge server. The transport's live acceptance against real `agy` is open, and `schema` remains the default until it passes; see `ROADMAP.md` §3.
