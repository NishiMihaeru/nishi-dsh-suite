...
Isolation therefore rests on `BLOCKED_NATIVE_TOOLS`, the post-hoc check on the vendor's own event log, with one exemption: `call_mcp_tool`, which on this transport is how a DSH tool is reached at all. That exemption was a defect found by the live run -- the loop worked and the backstop then failed the turn -- and it is covered by a test asserting that `run_command` in the same position still trips the check. Note also that `init.tools` reports all 57 native tools regardless of what an agent asks for, on both transports; whether that reflects the vendor's registry or the agent's effective set is unsettled, and `ROADMAP.md` §3 records it as an open question rather than a claim.
