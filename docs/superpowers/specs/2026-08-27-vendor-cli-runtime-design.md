...
5. Missing or incompatible vendor executables disable only their corresponding integration and do not prevent DSH startup.
...
`0.1.0-rc.2` does not pin or bundle one exact Codex package version. The runtime boundary is the official installed `codex` executable and its App Server protocol.

The CachyOS acceptance baseline for this RC is the official standalone Codex CLI `0.150.0`, resolved from `PATH`. This is the version that must pass the release live checks before publication.

The plugin must not reject a Codex installation solely because its version string differs from `0.150.0`. Compatibility is established by executable discovery plus the existing App Server initialize/handshake and required RPC behavior. If the installed executable cannot satisfy that protocol contract, only the Codex primary/subagent/usage integration becomes unavailable with a stable diagnostic; the rest of Nishi DSH Suite remains usable.

The project may document a tested-version set, but must not silently download or downgrade Codex to force a match. Future Codex releases that change the protocol may require a future Nishi prerelease.
...
The plugin must not silently fall back to a bundled Codex package. If `codex` is unavailable or App Server compatibility fails, Codex primary/subagent registration must fail closed for that integration only with a stable missing/incompatible-client diagnostic.
...
On CachyOS/Linux with Node 24, DSH `0.1.1-rc.2`, official Codex CLI `0.150.0`, and the already installed Claude/Antigravity clients:
...
The smoke also used `/tmp/.pnpm-store/v11` as a temporary content-addressable store. If it exists and is the temporary smoke store, it may be removed as cache cleanup. This does not affect global vendor CLIs.
...
The current real `web` profile is intentionally left on `0.1.0-rc.1` until `rc.2` exists so it can be upgraded normally without hand-editing the profile. Its duplicate package-local Codex `0.147.0`, Codex SDK, and Claude Agent SDK are considered temporary rc.1-owned artifacts and must disappear from the profile graph after upgrade.
...
- Codex App Server compatibility tests proving a successful handshake is accepted and incompatible startup fails only the Codex integration;
...
- real `rc.1 -> rc.2` registry upgrade acceptance on CachyOS/Linux using global Codex `0.150.0`.
