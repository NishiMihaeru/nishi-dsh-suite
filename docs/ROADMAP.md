...
- [ ] **wire the `mcp-bridge` transport into the adapter.** Remaining, and the whole of what makes the flag do anything: an agent definition allowing `call_mcp_tool` + `finish` and nothing else (the probe's default agent had 57 native tools and used `view_file` on its own accord), dropping `--json-schema` on that path, creating the bridge host before the spawn and claiming the child by pid after it, and the turn flow itself — a parked vendor call becomes a DSH `tool_calls` reply, and the next request for that session resolves it from `options.messages` and keeps reading the same vendor turn, mirroring `packages/codex/src/codex-plugin-dsh/adapter.ts:580-620`. Plus a precondition check that fails loudly, naming the `agy mcp add` command, rather than falling back silently;
- [ ] **live acceptance for the `mcp-bridge` transport**, and only then a decision on which transport becomes the default. Note the sequencing risk accepted by the maintainer: Antigravity was otherwise complete but for its freeze, and this reopens the provider stage;
