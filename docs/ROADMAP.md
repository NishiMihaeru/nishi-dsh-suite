...
- [ ] **decide which transport becomes the default.** Acceptance passes, but `schema` is still the default and switching is a one-key config change either way. Worth weighing: the bridge removes the tools-versus-strict-schema conflict and the whole schema-rewriting apparatus, and costs a globally registered server. A maintainer decision, not an adapter one;
- [ ] **open question about isolation, inherited rather than introduced.** `init.tools` reports all 57 native tools regardless of what an agent's `tools:` list says, on both transports. That does not prove the allowlist is ineffective — the vendor may report its registry rather than the agent's effective set — but it does mean nothing in this repository has ever *observed* the allowlist restricting anything, while `ARCHITECTURE.md` describes it as prevention with `BLOCKED_NATIVE_TOOLS` as the backstop. One live turn asking a `finish`-only agent to read a file would settle it. Until then, treat the backstop as the enforcement;
