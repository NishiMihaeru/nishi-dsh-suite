...
- **accepted debt — no vendor runtime version verification.** Codex pins `0.150.0` and enforces it from the App Server handshake; Antigravity runs whatever `agy` is installed, at any version. `agy` exposes no handshake, so an equivalent gate would mean parsing `agy --version` and gating on a known-good range. Deliberately not done for now: the maintainer accepted the risk on 2026-08-30. Revisit if a vendor upgrade ever breaks a contract silently. **Re-priced 2026-09-01, upward.** When this was accepted, one measured vendor behaviour was load-bearing. The socket-and-token bridge added two more, so three now are: that `agy` passes its environment to the MCP servers it launches verbatim, that `agy mcp add --env` merges with that rather than replacing it, and that a blocked MCP call holds the vendor turn open. None is documented by the vendor and each was established by probe. A silent change to any one breaks the default transport, and with no version gate the first symptom is a failed turn in production rather than a refused start. The risk was accepted at a lower price than it now carries;
