...
- [ ] **live acceptance for the `mcp-bridge` transport**, and only then a decision on which transport becomes the default. Note the sequencing risk accepted by the maintainer: Antigravity was otherwise complete but for its freeze, and this reopens the provider stage. One vendor interaction is **unprobed and load-bearing**: the adapter keeps `--sandbox`, and whether a sandboxed `agy` can spawn its MCP server at all was never tested — the probes ran without it. If it cannot, the choice is between dropping the flag on this transport and abandoning it, and that is a security decision rather than a fix. Probe it before anything else in the live run;
