...
**Every finding has now been verified and dispositioned**: 15 confirmed and fixed, one rejected (`terminate()` does escalate -- the upstream contract says the grace period is for exactly that), and one referred to the maintainer as a contract decision rather than quietly flipped (`latestCheckpoint` fails closed on a prior response with no usable checkpoint, where a documented rebuild path exists; nothing records why failing closed was chosen). Both touched providers were re-run live afterwards.
