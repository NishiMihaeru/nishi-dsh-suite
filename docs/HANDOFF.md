...
**8 confirmed or reported findings are still open**, listed in that document with file references. They are the next work, and none of them is in the bridge: they are spread across project memory's lock release, the core registry's invalidation loop, four seams in the Codex adapter's continuation path, and several Antigravity lifecycle and usage paths.
