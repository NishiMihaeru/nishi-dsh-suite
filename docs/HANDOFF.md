...
**6 findings are still open**, listed in that document with file references. Project memory's lock-release race and the core registry's invalidation loop are closed. What remains is four confirmed seams in the Codex adapter's continuation path -- a leak on a throw outside its `try/finally`, a continuation that ignores the current step's cancellation, a turn timeout that spans DSH's own tool execution, and raw vendor stderr inherited by the host process -- plus six claims not yet verified across the Suite preset manager, Codex history, and Antigravity's lifecycle and usage paths.
