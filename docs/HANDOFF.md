...
1. **Independent validation, then freeze.** This is the only blocker for all four packages. Everything below is optional next to it. The reason to insist keeps getting stronger: **three** reviews that were deliberately not told the author's reasoning have now each found defects the author's own green test runs had missed. The third (`docs/verification/gemini/LATEST.md`) reviewed only the MCP bridge change set and found a blocking silent-degradation defect in the one property that change set was most confident about. Note what that review is not: its scope was one change set, its charter and the reading of its findings were both written by the code's author, and nobody outside this workspace has signed anything.
