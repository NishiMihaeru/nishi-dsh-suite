...
- `nishi-dsh-grok` — provider id `grok`, route `grok-cli`, primary adapter over the installed official Grok Build CLI: one short-lived headless process per DSH step continuing one vendor session, with Usage & Limits from ACP `_x.ai/billing`;
