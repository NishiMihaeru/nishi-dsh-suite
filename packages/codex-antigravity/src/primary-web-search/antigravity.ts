...
function structuredResult(result: AgyResult): unknown {
  ...
  if (typeof result.response !== 'string' || result.response.trim().length === 0) {
    throw new PrimaryWebSearchError('WEB_SEARCH_PROTOCOL', 'Antigravity web_search returned no structured output')
  }
  try {
...
export class AntigravitySearchBackend implements PrimarySearchBackend {
  constructor(private readonly ctx: Context, private readonly config: AntigravitySearchBackendConfig) {}

  async search(route: PrimarySearchRoute, request: PrimarySearchRequest, parentSignal: AbortSignal): Promise<unknown> {
    ...
    try {
      ...
      try {
        ...
        if (!stdin || !stdout) {
          throw new PrimaryWebSearchError('WEB_SEARCH_PROVIDER_ERROR', 'Antigravity web_search subprocess did not expose required stdio pipes')
        }
        stdin.on('error', () => {})
        stdout.on('error', () => {})
        ...
        const resultPromise = new Promise<AgyResult>((resolve, reject) => {
          ...
          const onAbort = (): void => {
            if (parentSignal.aborted) {
              fail(new PrimaryWebSearchError('WEB_SEARCH_ABORTED', 'Antigravity web_search was aborted'))
            } else {
              fail(new PrimaryWebSearchError('WEB_SEARCH_PROVIDER_ERROR', `Antigravity web_search timed out after ${this.config.timeoutMs}ms`))
            }
          }
          ...
          lines.on('line', (line) => {
            ...
            try {
              ...
              fail(new PrimaryWebSearchError('WEB_SEARCH_PROTOCOL', `Antigravity emitted non-JSON stdout in stream-json mode: ${bounded(trimmed)}`, { cause: error }))
              return
            }
          ...
          lines.on('close', () => {
            ...
            void child.done.then((outcome) => {
              ...
              fail(new PrimaryWebSearchError('WEB_SEARCH_PROVIDER_ERROR', `Antigravity web_search exited before a result event (exit ${String(outcome.exitCode)})${stderr ? `: ${bounded(stderr)}` : ''}`))
            }).catch(fail)
          })
        })
        resultPromise.catch(() => {})
        ...
        if (result.status !== 'SUCCESS') {
          throw new PrimaryWebSearchError('WEB_SEARCH_PROVIDER_ERROR', `Antigravity native web_search failed for model ${JSON.stringify(route.model)}: ${bounded(result.error || `status ${String(result.status)}`)}`)
        }
        const tools = nativeToolNames(events)
        if (!tools.includes('search_web')) {
          throw new PrimaryWebSearchError('WEB_SEARCH_PROTOCOL', `Antigravity model ${JSON.stringify(route.model)} completed the hidden search turn without search_web`)
        }
        const unexpected = tools.filter(name => name !== 'search_web' && name !== 'finish')
        if (unexpected.length > 0) {
          throw new PrimaryWebSearchError('WEB_SEARCH_PROTOCOL', `Antigravity hidden search turn invoked unexpected native tool(s): ${unexpected.join(', ')}`)
        }
