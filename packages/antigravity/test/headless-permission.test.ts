...
import { headlessPermissionDenial } from '../src/antigravity-subagent.ts'
...
test('recognises the headless permission denial and names the tool', () => {
  const diagnostic = headlessPermissionDenial(REAL_STDERR)
...
test('leaves unrelated failures to the generic diagnostic', () => {
  assert.equal(headlessPermissionDenial(undefined), undefined)
  assert.equal(headlessPermissionDenial(''), undefined)
  assert.equal(headlessPermissionDenial('some unrelated vendor crash'), undefined)
})
