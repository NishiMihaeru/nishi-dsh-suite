...
Detecting it is not the end of the step. `--json-schema` turns out not to constrain the model at all: measured on `agy 1.1.25`, the vendor asks for a JSON block appended to the model's own answer and parses it back out of `response` — undeclared fields the model wrote come back filtered off, which constrained decoding could not do. A turn with no block is therefore the model forgetting, not the conversation breaking, and the child is measurably fine afterwards. So the adapter asks it to say that turn again: a `repair` envelope carrying the lost stamp, a fresh stamp of its own and **no DSH history**, which is what makes it a restatement rather than a retry — no tool runs twice. Once per step and never twice; a second stale reply fails the step and still keeps the child.
