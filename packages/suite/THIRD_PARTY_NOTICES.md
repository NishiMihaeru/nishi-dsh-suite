...
`codex-plugin-dsh` provides the `codex-app-server` provider adapter that integrates with the local Codex App Server. It is an open-source external plugin by wingoo and is not an official OpenAI plugin.
