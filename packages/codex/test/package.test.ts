...
test('Codex package manifest has independent public identity without bundled vendor runtimes', async () => {
  ...
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/dsh-session'], '0.1.2-alpha.1')
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/dsh-attachment'], '0.1.2-alpha.1')
