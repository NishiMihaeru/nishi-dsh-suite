...
`0.1.2-alpha.1` is the only supported DSH generation for this suite. Codex's own evidence for it is executable, not inherited: 78 unit tests plus a full 15-scenario live acceptance against real `codex-cli 0.150.0` processes, both on the alpha.1 baseline.
...
One defect is open against this package, found by a delegation run the same day: a turn whose first step runs on another provider and emits tool calls fails on the step that consumes those tool results here, with `codex-plugin-dsh: the current Codex turn has no user input`. The session recovers on its next turn. See `docs/ROADMAP.md` §2.
