...
test('known collector failures become safe non-active snapshots', async () => {
  ...
  const codex = await new CodexUsageCollector({
    async readRateLimits() { throw new CodexRateLimitsSourceError('raw credential path must not escape', 'LOGIN_REQUIRED') },
  }).collect(observedAtMs)
  const claude = await new ClaudeUsageCollector({
    async getUsage() { throw new ClaudeUsageSourceError('vendor stderr must not escape', 'UNAVAILABLE') },
  }).collect(observedAtMs)
  const antigravity = await new AntigravityUsageCollector({
    async readCapability() { throw new AntigravityUsageSourceError('agy raw failure must not escape', 'UNSUPPORTED') },
  }).collect(observedAtMs)
...
test('unknown collector failures collapse to ERROR without raw error details', async () => {
  const snapshots = await Promise.all([
    new CodexUsageCollector({ async readRateLimits() { throw new Error('codex secret stderr') } }).collect(1),
    new ClaudeUsageCollector({ async getUsage() { throw new Error('claude secret stderr') } }).collect(1),
    new AntigravityUsageCollector({ async readCapability() { throw new Error('agy secret stderr') } }).collect(1),
  ])
